...
CREATE TABLE IF NOT EXISTS platform_deposits (
  ...
  user VARCHAR(44) NOT NULL, -- User's wallet address
...
CREATE INDEX IF NOT EXISTS idx_platform_deposits_user ON platform_deposits(user);
...
CREATE POLICY "User can update own deposits" ON platform_deposits
  FOR UPDATE
  USING (user = current_setting('request.jwt.claims', true)::json->>'sub');
...
COMMENT ON COLUMN platform_deposits.user IS 'User wallet address who made the deposit';
