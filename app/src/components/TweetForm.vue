...
<template>
  <div v-if="connected" class="card mb-6 group relative overflow-hidden">
    ...
    <div class="relative z-10">
      <!-- Treasury Info -->
      <div class="mb-3 md:mb-4 p-2 md:p-3 bg-primary-500/10 border border-primary-500/20 rounded-lg text-primary-300 text-xs md:text-sm">
        <div class="flex items-center space-x-2">
          <svg class="w-3 h-3 md:w-4 md:h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
          </svg>
          <span class="hidden md:inline">Each beacon costs 0.001 SOL and creates an on-chain transaction. Transaction signature becomes your beacon ID!</span>
          <span class="md:hidden">Beacon costs 0.001 SOL. Transaction signature = beacon ID!</span>
        </div>
      </div>
      ...
      <div class="flex flex-col md:flex-row md:flex-wrap items-stretch md:items-center justify-between space-y-3 md:space-y-0">
        <!-- Enhanced Topic field -->
        <div class="relative flex-1 md:max-w-xs group/topic">
          <input
            type="text"
            placeholder="Add a topic..."
            class="input-field pl-8 md:pl-10 pr-4 py-2 text-sm transition-all duration-300 focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 w-full"
            :class="{}"
            :value="effectiveTopic"
            :disabled="forcedTopic != null"
            @input="handleTopicChange"
          />
          <div class="absolute left-0 inset-y-0 flex pl-2 md:pl-3 pr-2">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4 md:h-5 md:w-5 m-auto transition-all duration-300"
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-4">
          ...
          <div v-if="connected && platformWalletAddress" class="flex items-center space-x-2 md:space-x-3 text-xs md:text-sm">
            <span class="text-dark-300 hidden md:inline">Payment:</span>
            <span class="text-dark-300 md:hidden">Pay:</span>
            <div class="flex items-center space-x-1 md:space-x-2">
              ...
              <span class="text-yellow-400 font-medium text-xs md:text-sm">Smart</span>
              <span class="text-xs text-dark-400 hidden md:inline">
                (Platform: {{ platformBalance.toFixed(4) }} SOL → Phantom fallback)
              </span>
              <span class="text-xs text-dark-400 md:hidden">
                ({{ platformBalance.toFixed(3) }} SOL)
              </span>
            </div>
          </div>
