  ...
  const buyCA = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const purchaseType = result.purchaseType === 'SMART_PAYMENT' ? 'Smart Payment (Platform + User)' : 'User Wallet (Solana Tracker)';
        const message = result.purchaseType === 'SMART_PAYMENT' 
          ? `✅ ${purchaseType} successful! Platform wallet paid, you received CA tokens! Swap: ${result.swapSignature?.slice(0, 8)}...`
          : `✅ ${purchaseType} successful! SOL swapped for CA tokens via Solana Tracker (supports Pump.fun!). You now have CA tokens in your wallet! Transaction: ${result.swapSignature?.slice(0, 8)}...`;
        caBuyError.value = message;
...
<template>
  <div class="card group relative overflow-hidden" :data-tweet-id="tweet?.id">
    ...
    <!-- Inline Buy CA Box - Like Reply System -->
    <div v-if="showBuyCAModal" class="mt-4 p-4 bg-dark-800/50 border border-dark-700 rounded-lg">
      <div class="flex flex-col space-y-4">
        ...
        <div>
          ...
          <div class="relative">
            <input
              ...
              min="0.01"
              placeholder="0.1"
              class="w-full px-3 py-2 bg-dark-800 border border-dark-600 rounded text-white placeholder-dark-400 focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500"
            />
        ...
        <!-- Error Message -->
        <div v-if="caBuyError" class="p-3 bg-red-500/10 border border-red-500/20 rounded">
          <div class="flex items-center space-x-2">
        ...
        <div class="flex justify-center space-x-3">
          ...
          <button 
            ...
            class="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white rounded transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <div class="flex items-center space-x-2">
