  ...
  <div class="card group relative overflow-hidden">
    ...
    <div class="flex items-start space-x-3 md:space-x-4 relative z-10">
      ...
      <div class="flex-1 min-w-0">
        <!-- Enhanced Header -->
        <div class="flex flex-col md:flex-row md:items-center space-y-1 md:space-y-0 md:space-x-2 mb-2 md:mb-3">
          ...
          <div class="flex items-center space-x-1">
            <svg class="w-3 h-3 md:w-4 md:h-4 text-primary-400" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between mt-3 md:mt-4 space-y-3 md:space-y-0">
          <!-- Left side: Action buttons -->
          <div class="flex items-center justify-between md:justify-start space-x-4 md:space-x-6">
            <button 
              @click="handleLike"
              :class="[
                'flex items-center space-x-1 md:space-x-2 transition-colors duration-300 hover:scale-110',
            ...
            <button 
              @click="handleReply"
              class="flex items-center space-x-1 md:space-x-2 text-dark-400 hover:text-blue-400 transition-colors duration-300 hover:scale-110"
            >
              <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
              </svg>
              <span class="text-xs md:text-sm">{{ showReplies ? 'Reply' : 'Replies' }}</span>
            </button>
            <button 
              @click="handleTip"
              class="flex items-center space-x-1 md:space-x-2 text-dark-400 hover:text-yellow-400 transition-colors duration-300 hover:scale-110"
            >
              <svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-xs md:text-sm">Tip</span>
            </button>
            <button 
              @click="handleShareLink"
              class="flex items-center space-x-1 md:space-x-2 text-dark-400 hover:text-green-400 transition-colors duration-300 hover:scale-110"
          ...
          <div v-if="totalTips > 0 || loadingTips" class="flex items-center space-x-1 md:space-x-2 text-yellow-400">
            <svg class="w-3 h-3 md:w-4 md:h-4" fill="currentColor" viewBox="0 0 20 20">
              <path d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z" />
            </svg>
            <span v-if="loadingTips" class="text-xs md:text-sm text-dark-400">Loading...</span>
            <span v-else class="text-xs md:text-sm font-semibold">
              {{ totalTips.toFixed(3) }} SOL
              <span v-if="tipCount > 1" class="text-dark-400 text-xs ml-1 hidden md:inline">({{ tipCount }} tips)</span>
            </span>
          </div>
        </div>
      </div>
